...
//	cout << firstday;
	if (cday < firstday) {
		...
		if (date_f - 1 == 4) {
			cout << start << "到" << 20150228<< "利息为：" << gg << endl;
		}
		else {
			cout << start << "到" << (int)ll[date_f - 1][0] - 1 << "利息为：" << gg << endl;
		}
...
//		cout << endl <<gg<< sy;
		...
		while (sy != 0) {
			if (sy < ll[date_f - 1][8]) {
				...
				if (date_f - 1 == 4) {
					cout <<(int)ll[date_f ][0] << "到" << 20150228 << "利息为：" <<loans*ll[date_f][8] * ll[date_f][type_f] / 100 / 365<< endl;
				}
				else {
					cout << (int)ll[date_f ][0]<< "到" << (int)ll[date_f - 1][0]-1  << "利息为：" << loans*ll[date_f][8] * ll[date_f][type_f] / 100 / 365 << endl;
				}
